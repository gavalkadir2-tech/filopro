...
**Groq**'u kullanır — hızlı, açık modelleri (Llama 3.3 70B) kredi kartı
gerektirmeden, ücretsiz bir kotayla sunan bir sağlayıcı. API anahtarı
güvenlik nedeniyle **tarayıcıda değil, bu sunucuda** saklanır ve
`/api/ai/chat` ucu üzerinden vekil (proxy) olarak kullanılır.

1. [console.groq.com](https://console.groq.com) adresine gidin, ücretsiz
   hesap oluşturun (kredi kartı istenmez).
2. **"API Keys"** bölümünden yeni bir anahtar oluşturun (`gsk_...` ile
   başlar).
3. `.env` dosyasına ekleyin: `GROQ_API_KEY=gsk_...`
4. İsteğe bağlı olarak `AI_DAILY_LIMIT` ile şirket başına günlük istek
   sınırı belirleyin (varsayılan: 200) — Groq'un ücretsiz kotası da kendi
   içinde sınırlı olduğundan, kötüye kullanımı önlemek için faydalıdır.

Bu anahtar tanımlı değilse sadece AI modülü çalışmaz, senkronizasyon ve
