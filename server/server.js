...
// için bu uca istek atar; anahtar sadece burada, sunucuda kalır. Sadece giriş
// yapmış (JWT'li) kullanıcılar çağırabilir, ayrıca kaba bir günlük kullanım
// sınırı vardır (maliyet kontrolü için).
const AI_DAILY_LIMIT = parseInt(process.env.AI_DAILY_LIMIT || '200', 10); // tenant başına/gün
const aiUsage = new Map(); // tenantId -> {count, day}
function aiRateLimit(req, res, next) {
  const day = new Date().toISOString().slice(0, 10);
  const key = req.user.tenantId;
  const rec = aiUsage.get(key) || { count: 0, day };
  if (rec.day !== day) {
    rec.count = 0;
    rec.day = day;
  }
  if (rec.count >= AI_DAILY_LIMIT) {
    return res.status(429).json({ error: 'Günlük yapay zeka kullanım sınırına ulaşıldı. Yarın tekrar deneyin.' });
  }
  rec.count++;
  aiUsage.set(key, rec);
  next();
...
app.post('/api/ai/chat', authMiddleware, aiRateLimit, async (req, res) => {
  if (!GROQ_API_KEY) {
    return res.status(503).json({ error: 'Sunucuda GROQ_API_KEY tanımlı değil. Yapay zeka özellikleri devre dışı.' });
  }
